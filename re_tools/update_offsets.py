...
def resolve_symbol(pe_image: PeImage, symbol_name: str, symbol_config: dict[str, Any]) -> MatchResult:
    ...
    if resolver == "pattern_start_rva":
        ...
        value: int = pe_image.file_offset_to_rva(file_offset)
    ...
    if resolver == "rip_relative_rva":
        ...
        _, target_section = pe_image.rva_to_file_offset(value)
        return MatchResult(symbol_name, value, target_section.name, displacement_file_offset)
...
def update_header_text(header_text: str, resolved_values: dict[str, int]) -> tuple[str, list[str]]:
    ...
    for symbol_name, value in resolved_values.items():
        pattern: re.Pattern[str] = re.compile(
            rf"(^\s*#define\s+{re.escape(symbol_name)}\s+)0x[0-9A-Fa-f]+([uUlL]*)(.*$)",
            re.MULTILINE
        )
        ...
        def replace(match: re.Match[str]) -> str:
            ...
            return f"{match.group(1)}0x{value:06X}{suffix}{match.group(3)}"
...
def main() -> int:
    parser: argparse.ArgumentParser = argparse.ArgumentParser(
        description="Update Mewgenics mod RVAs/data offsets in game_runtime_types.h from byte signatures..."
    )

    parser.add_argument("exe_path", type=Path, help="Path to the updated Mewgenics.exe...")
    parser.add_argument("header_path", type=Path, help="Path to game_runtime_types.h...")
    parser.add_argument("--signatures", type=Path, default=Path("signatures.json"), help="Path to signatures.json...")
    ...
    for symbol_name, symbol_config_any in signature_config["symbols"].items():
        ...
        print(f"{symbol_name} = 0x{result.value:06X} ({result.section_name}, file+0x{result.file_offset:X})")
